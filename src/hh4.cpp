...
// Какие кнопки нажимать в зависимости от направлений x_direction+1,
// y_direction+1;
static int button_pressed[3][3] = {{7, 6, 5}, {0, -1, 4}, {1, 2, 3}};
...
int MHookHandler4::OnMouseMove(LONG _x, LONG _y) {
  ...
  if ((initialized) && (!rbutton_pressed))  // известно последнее положение
                                            // мыши, правая кнопка не нажата
  {
    dx = (_x - last_x);
    dy = (_y - last_y);
    ...
    if ((true == MHSettings::flag_2moves) && (0 != MHSettings::mode3axe)) {
      ...
      if (0 != x_limitation) {
        if ((0 == last_x_direction)
            && (thistime - steps2timeout
                > 50))  // Можно снимать ограничения, выждали 100 мс в середине
        {
          x_limitation = 0;
        } else  // Ограничения продолжают действовать, таймер перевзводится, так
                // как мышь двигалась
        {
    ...
    if (0 == MHSettings::mode3axe)  // Не работает ли ось Х по режиму 3?
    {
      ...
      // Это работает частично, начало и конец быстрого движения не ловятся, как
      // в режиме 3
    ...
    } else  // Ось X работает в режиме 4
    {
      ...
      if (X < -3 * MHSettings::deadx / 2)  // вышло за границы слева
      {
        ...
        X = -3 * MHSettings::deadx / 2;
                 ...
                 < -MHSettings::deadx / 2)  // в пределах границ, направление -1
      {
        x_direction = -1;
      ...
      } else if (X > MHSettings::deadx / 2)  // в пределах границ, направление 1
      {
        ...
        x_direction = 0;  // в пределах мертвой зоны

      if ((true == MHSettings::flag_2moves) && (0 != x_direction))
        x_limitation = x_direction;
    }
    ...
    if (1 == MHSettings::mode3axe)  // Не работает ли ось Y по режиму 3?
    {
      ...
      if (Y < -3 * MHSettings::deady / 2)  // вышло за границы слева
      {
        ...
        Y = -3 * MHSettings::deady / 2;
                 ...
                 < -MHSettings::deady / 2)  // в пределах границ, направление -1
      {
        y_direction = -1;
      ...
      } else if (Y > MHSettings::deady / 2)  // в пределах границ, направление 1
      {
        ...
        y_direction = 0;  // в пределах мертвой зоны
    }
    /*
                    // А может в два движения? (ось X не должна работать в
       режиме 3) if((true==MHSettings::flag_2moves)&&(0!=MHSettings::mode3axe))
                    {
                            if((1==last_x_direction)&&(1!=x_direction))
       last_right=timeGetTime(); // Ушли из правого положения в это время
                            if((-1==last_x_direction)&&(-1!=x_direction))
       last_left=timeGetTime(); // Ушли из левого положения в это время

                            // Можно ли занять новое положение? Прошло ли 100 мс
       с последнего движения? if((-1==x_direction)&&(-1!=last_x_direction)) //
       Рано ещё, иди в центр и жди без движения 100 мс
                            {
                                    X=0;
                                    x_direction=0;
    ...
    */
    if ((last_y_direction != y_direction)
        || (last_x_direction != x_direction))  // Нажатые кнопки поменялись
    {
