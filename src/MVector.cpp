...
// Если оно в другом направлении - ждём, когда вектор вырастет до предела
// переключения Возвращает новый номер позиции или -1, если позиция не
// изменилась или -2, если мышь подвинули недостаточно В переменные quad_dx и
// quad_y возвращаем вектор, который соответствует накопленному движению мыши
// (нормализованный)
//===========================================================================================================
int MHVector::NewValues(LONG dx, LONG dy) {
  ...
  /*
          // Вводим новую проверку! Остаток от предыдущего движения игнорируем,
     если он не попадает в тот же румб,
  ...
  */
  ...
  if (4 == MHSettings::GetNumPositions()) {
    // Начало отсчёта будет -135 градусов и пойдёт по часовой стрелке (левая
    // граница нулевой кнопки)
    angle = angle + 135;
    if (angle < 0) angle += 360;
    new_position = angle / 90;  // один из четырёх румбов
    if (new_position > 3)
      vector_position = 3;  // Вдруг ошибки округления? Не стал много думать
  } else                    // восемь румбов
  {
    // Начало отсчёта будет -112 (вообщето, с половиной, но забьём) градусов и
    // по часовой стрелке (левая граница нулевой кнопки)
    angle = angle + 112;
    if (angle < 0) angle += 360;
    new_position = angle / 45;  // один из восьми румбов
    if (new_position > 7)
      vector_position = 7;  // Вдруг ошибки округления? Не стал много думать
  }
