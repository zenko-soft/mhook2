...
int MHookHandler1a::OnMouseMove(LONG _x, LONG _y) {
                                            ...
                                            // мыши, правая кнопка не нажата
  {
    ...
    // Поэтому не 0<=..., а -1<=...
    if (-1 <= position)  // -2=мышь ваще не двигалась, -1= направление не
                         // изменилось
    {
      time_now = timeGetTime();

      // если направление изменилось - время с последнего нажатия не имеет
      // значения если не изменилось -  важно, чтобы с момента последнего
      // нажатия прошло достаточно времени
      if ((0 <= position)
          || (time_now - last_time > MHSettings::time_between_pushes)) {
