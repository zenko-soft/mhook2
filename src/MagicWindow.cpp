...
extern bool G_dialogue2_active;  // чтобы окна могли писать на лету в диалог
                                 // конфигурации
...
RECT MagicWindow::adjust_rect = {
    0};  // корректировка размеров окна с учетом заголовка и рамки
...
MagicWindow MagicWindow::magic_wnd[NUM_MAGIC_WINDOWS] = {
    {0,
     ...
     false,
     0,
     {sizeof(TRACKMOUSEEVENT), TME_LEAVE, 0, HOVER_DEFAULT}},  // q
    {1,
     0,
     0,
     L"Окно 2",
     ...
     false,
     0,
     {sizeof(TRACKMOUSEEVENT), TME_LEAVE, 0, HOVER_DEFAULT}},  // e
    {2,
     0,
     0,
     L"Окно 3",
...
// static TRACKMOUSEEVENT
// tme={sizeof(TRACKMOUSEEVENT),TME_LEAVE,0,HOVER_DEFAULT};
  ...
  switch (message) {
    ...
    case WM_MOVE:
      ...
      if (MagicWindow::editmode)
        mw->x += MagicWindow::adjust_rect.left;  // horizontal position
    ...
    case WM_SIZE:
      ...
      // area. The high-order word of lParam specifies the new height of the
      // client area.
      mw = (MagicWindow *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
      mw->width = (int)(short)LOWORD(lparam);
      if (MagicWindow::editmode)
        mw->width = mw->width - MagicWindow::adjust_rect.left
                    + MagicWindow::adjust_rect.right;  // horizontal position
      mw->height = (int)(short)HIWORD(lparam);
      if (MagicWindow::editmode)
        mw->height = mw->height - MagicWindow::adjust_rect.top
                     + MagicWindow::adjust_rect.bottom;
      if (G_dialogue2_active)  // Прямо в диалоге меняем циферки
      {
        TCHAR unicode_buf[80];
        _itow_s(MagicWindow::magic_wnd[mw->myindex].width, unicode_buf, 10);
        SendDlgItemMessage(Settings2HWND,
                           IDC_EDIT_WWIDTHS[mw->myindex],
                           WM_SETTEXT,
                           0,
                           (LPARAM)unicode_buf);
        _itow_s(MagicWindow::magic_wnd[mw->myindex].height, unicode_buf, 10);
        SendDlgItemMessage(Settings2HWND,
                           IDC_EDIT_WHEIGHTS[mw->myindex],
                           WM_SETTEXT,
                           0,
                           (LPARAM)unicode_buf);
      }
    ...
    case WM_MOUSEMOVE:
      mw = (MagicWindow *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
      if (mw->mouse_or_eytracker != 0)
        break;  // Только мышиные окна обрабатываем!
      if ((!MagicWindow::editmode) && (!(mw->f_inside_window))) {
        ...
        if (mw->pressed)
          ...
          SetLayeredWindowAttributes(hwnd, NULL, 255 * 75 / 100, LWA_ALPHA);

        // При нажатии, если уже нажата другая кнопка в группе, отжать её (кроме
        // самой себя)!
    ...
    case WM_MOUSELEAVE:
      ...
      if (!MagicWindow::editmode) {
        ...
        if (0
            == mw->button_or_switch)  // Это кнопка, её надо отпустить.
                                      // Переключатель при покидании не трогаем.
        {
          // Возвращаем прозрачностьeqqeeqqqe
          SetLayeredWindowAttributes(hwnd, NULL, 255 * 25 / 100, LWA_ALPHA);
          // Эта проверка возникла из-за ситуации, когда нажимаем курсор
          // мыши/скрываем окно
...
void MagicWindow::Init() {
  ...
  for (i = 0; i < NUM_MAGIC_WINDOWS; i++) {
    ...
    magic_wnd[i].MWhwnd = CreateWindowEx(
        ...
        MHInst,
        //(LPVOID)1L
        (LPVOID)&magic_wnd[i]  // укажем на объект !!! Далее - враньё, всё
                               // работает. (В 64-битной версии укзатель
                               // обрезается, приходится делать SetWindowLong
                               // (ниже))
        // 0
    );
...
void MagicWindow::Hide() {
  ...
  editmode = true;  // Клавиши не нажимаются (для айтрекера, мышь и так в них не
                    // попадёт)

  for (i = 0; i < NUM_MAGIC_WINDOWS; i++) {
    if (magic_wnd[i].active) ShowWindow(magic_wnd[i].MWhwnd, SW_HIDE);
    if (magic_wnd[i].pressed)
      magic_wnd[i].Press();  // Отпускаем все нажатые клавиши
...
void MagicWindow::ShowEditable() {
  ...
  for (i = 0; i < NUM_MAGIC_WINDOWS; i++) {
    ...
    SetWindowLongPtr(
        magic_wnd[i].MWhwnd, GWL_STYLE, WS_CAPTION | WS_THICKFRAME);
...
void MagicWindow::ShowRuntime() {
  ...
  if (timer5_needed && MHhwnd)
    SetTimer(MHhwnd, 5, 25, NULL);  // 40 раз в секунду
}
...
void MagicWindow::Press() {
  BYTE lobyte = LOBYTE(dlg_scancodes[button_index].value),
       hibyte = HIBYTE(dlg_scancodes[button_index].value);
  ...
  // Специальные значения клавиш: E1-в старшем байте + число, обозначающее
  // операцию
  if (0xE1 == hibyte) {
    PressSpecial(lobyte);
    return;
  ...
  if (dlg_scancodes[button_index].value
...
void MagicWindow::PressSpecial(BYTE operation) {
  switch (operation) {
    ...
    {
      if (false == pressed)
        return;  // Щелкаем только при вхождении, а не при выходе
      ...
      flag_ignore_mouse_move =
          1;  // При спрятывании и появлении окна генерируется ложное mouse_move
    ...
    case 4:  // Колёсико мыши едет туда
      if (false == pressed)
        mouse_auto_w_direction = 0;
      else
        mouse_auto_w_direction = 1;
      break;
...
void MagicWindow::OnTimer5() {
  if (editmode)
    return;  // на всякий пожарный. таймер не должен работать в режиме
             // редактирования
