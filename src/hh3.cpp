...
static int offset = 0, last_any_position = -1, serial_fasts = 0,
           oast_allowed = 0, b2st_allowed = 0;

static DWORD last_any_time = 0,  // Время последнего пперемещения мыши
    last_fast_time = 0, last_bast_time = 0;
...
int MHookHandler3::OnMouseMove(LONG _x, LONG _y) {
  ...
  // if((initialized)&&(!rbutton_pressed)) // известно последнее положение мыши,
  // правая кнопка не нажата
  if (initialized)  // известно последнее положение мыши, правая кнопка теперь
                    // тоже генерирует клавиши
  {
    dx = (_x - last_x);
    dy = (_y - last_y);
    ...
    if (!MHSettings::flag_skip_fast) {
      ...
      if (0 <= position)  // -2=мышь подвинулась на недостаточное растояние, -1=
    ...
    } else  // а вот здесь нажимаем не всегда
    {
      ...
      Qspeed = 100.0f * (dx * dx + dy * dy)
               / (time_now - last_any_time);  // пикселов в квадрате за 100 мс
      if (Qspeed > MHSettings::minimal_mouse_speed)  // Это элемент быстрого
                                                     // движения!!!
      {
        serial_fasts++;
        if (serial_fasts > 2)
          ...
          oast_allowed =
              0;  // Мы достаточно набрали fast, чтобы игнорировать хвост
        ...
        swprintf(debug_buf,
                 _countof(debug_buf),
                 L"FAST dx: %d  dy: %d Qspeed: %f time: %lu\n",
                 dx,
                 dy,
                 Qspeed,
                 time_now - last_any_time);
      ...
      } else  // Скорость низкая, проверяем дополнительно
      {
        ...
        if ((time_now - last_fast_time < 80) && (oast_allowed)) {
          ...
          swprintf(debug_buf,
                   _countof(debug_buf),
                   L"OAST dx: %d  dy: %d Qspeed: %f time: %lu ftime: %lu\n",
                   dx,
                   dy,
                   Qspeed,
                   time_now - last_any_time,
                   time_now - last_fast_time);
          OutputDebugString(debug_buf);
#endif
        }
        // 2.2 а,б - начало, запоминаем его время
        // else
        // if(((Qspeed<2.0f)&&(time_now-last_any_time>30))||((0==dx)&&(0==dy)))
          ...
          swprintf(debug_buf,
                   _countof(debug_buf),
                   L"BAST dx: %d  dy: %d Qspeed: %f time: %lu ftime: %lu\n",
                   dx,
                   dy,
                   Qspeed,
                   time_now - last_any_time,
        ...
        // 2.2. в: возможно, разгоняемся
          ...
          swprintf(debug_buf,
                   _countof(debug_buf),
                   L"B2ST dx: %d  dy: %d Qspeed: %f time: %lu ftime: %lu\n",
                   dx,
                   dy,
                   Qspeed,
                   time_now - last_any_time,
          ...
          OutputDebugString(debug_buf);
#endif
        } else  // Всё-таки настояли на SLOW! (теперь сюда попадают и dx==0,
                // dy==0)
        {
          ...
          if (IsOp)  // Поменяли направление - нужно считать этот отсчёт - BAST
                     // (B-ST в диагностике)
          {
            last_bast_time = time_now;
            b2st_allowed = 2;
            MHVector::Reset();  // Вот это обязательно, иначе в том же
                                // направлении мышь не нажмёт клавишу
            InvalidateRect(MHhwnd, NULL, TRUE);  // Правильные квадратики
#ifdef _DEBUG
            swprintf(debug_buf,
                     _countof(debug_buf),
                     L"OPPO dx: %d  dy: %d Qspeed: %f time: %lu ftime: %lu\n",
                     dx,
                     dy,
                     Qspeed,
                     time_now - last_any_time,
            ...
            if (0 <= position)  // -2=мышь подвинулась на недостаточное
                                // растояние, -1= направление не изменилось
            {
              MHKeypad::Press(position, true, offset);
              position_mem = position;
            }
            ...
            swprintf(debug_buf,
                     _countof(debug_buf),
                     L"SLOW dx: %d  dy: %d Qspeed: %f time: %lu ftime: %lu "
                     L"pos: %d IsOp: %d\n",
                     dx,
                     dy,
                     Qspeed,
    ...
    /*
                    // Теперь берём цифры из поля  "быстрое движение - это со
    скоростью, большей..."
                    // Цифры не настоящие, мы меряем не скорость, а один отсчёт,
    не глядя на время.
                    ...
                    {
    #ifdef _DEBUG
                    //	swprintf(debug_buf,_countof(debug_buf),L"SLOW: dx: %d
    dy: %d time: %ul\n", dx,dy,time_now-last_fast_time);
                    //	OutputDebugString(debug_buf);
    #endif
                            // нажатие разрешено при flag_skip_test только, если
    успокоились после быстрого движения
                            //if((!MHSettings::flag_skip_fast)||(time_now-last_fast_time>1000))
                            {
                                    position=MHVector::NewValues(dx,dy);

    #ifdef _DEBUG
                                    //swprintf(debug_buf,_countof(debug_buf),L"SLOW
    dx: %d  dy: %d Qspeed: %f time: %lu pos: %d\n",
    dx,dy,sqrt((float)(dx*dx+dy*dy))/(time_now-last_any_time),
    time_now-last_fast_time, position);
                                    //swprintf(debug_buf,_countof(debug_buf),L"SLOW
    dx: %d  dy: %d Qspeed: %f time: %lu pos: %d\n",
    dx,dy,5625.0*(dx*dx+dy*dy)/(time_now-last_any_time)/(time_now-last_any_time),
    time_now-last_fast_time, position);
                                    swprintf(debug_buf,_countof(debug_buf),L"SLOW
    dx: %d  dy: %d Qspeed: %f time: %lu ftime: %lu pos: %d\n",
    dx,dy,100.0f*(dx*dx+dy*dy)/(time_now-last_any_time), time_now-last_any_time,
    time_now-last_fast_time, position); OutputDebugString(debug_buf); #endif
                                    if(0<=position) // -2=мышь подвинулась на
    недостаточное растояние, -1= направление не изменилось
                                    {
                                            MHKeypad::Press(position,true,offset);
                                            position_mem=position;
                                    }
                                    // Таймер взводим заново при любом движении
    мыши, если было хоть что-то нажато ранее
