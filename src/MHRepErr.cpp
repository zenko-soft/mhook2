...
void MHReportError(const wchar_t *SourceFile,
  ...
  TCHAR BKBMessage[1024];  // Это строка, в которой формируется сообщение об
                           // ошибке
  ...
  if (BKBLastError != (DWORD)0)  // Получить строку, если код не равен нулю
  {
    res = FormatMessage(
        ...
        (LPTSTR)&BKBStringError,
        0,
        NULL);

    if (res == (DWORD)0)
      BKBStringError = (void *)L"Сообщение об ошибке не найдено";
  } else {
  ...
  MessageBox(
      hwnd, BKBMessage, L"MH: сообщение об ошибке", MB_OK | MB_ICONINFORMATION);
}
  ...
  if (tbg_error_code) {
    ...
    if (fp_tobiigaze_get_error_message) {
      MultiByteToWideChar(CP_ACP,
                          0,
                          (*fp_tobiigaze_get_error_message)(tbg_error_code),
                          -1,
                          ConvertASCII2W,
                          1023);
      tmp_char = ConvertASCII2W;
    } else
      tmp_char = L"неизвестно";

    // Сформировать строку с полным описанием ошибки
    swprintf_s(ARCMessage,
               _countof(ARCMessage),
               L"Module: %s\nFunction: %s\nLine number: %d\nОшибка Tobii Gaze "
               L"SDK: %d (%s)",
               SourceFile,
    ...
    MessageBox(hwnd,
               ARCMessage,
               L"MHook:Gaze SDK: сообщение об ошибке",
