...
// У Саши Зенько роль правой кнопки выполняет колёсико!!! А настоящая правая
// делает только скролл Потом вынести это на экран параметров
#undef SASHA_ZENKO

extern bool flag_magic_left_click;  // Клик мыши произведён волшебным окном, не
                                    // отключать его в HookProc,

extern HWND MHhwnd;  // Нужна для установки таймера
extern LONG screen_x_real, screen_y_real;  // Для определения углов экрана
extern bool flag_inside_window;  // Определён в оконной процедуре, показывает,
                                 // что мы внутри окна
...
int top_position =
    -1;  // 0 - левый верхний угол, 1- правый верхний угол, -1 - убрали
    ...
    if (pMouseStruct != NULL) {
      switch (wParam) {
        case WM_MOUSEMOVE:
          ...
          // if(pMouseStruct->pt.x+pMouseStruct->pt.y<5) // А находимся ли мы в
          // верхнем левом углу экрана?
          if ((pMouseStruct->pt.y - pMouseStruct->pt.x
               > (screen_y_real - 1) - 5)
              && (pMouseStruct->pt.x < screen_x_real)
              && (pMouseStruct->pt.y
                  < screen_y_real))  // А находимся ли мы в нижнем левом углу
                                     // экрана? (но не на втором экране)
          {
          ...
          // else if(pMouseStruct->pt.x-pMouseStruct->pt.y>screen_x-5) // Правый
          // верхний угол
          else if ((pMouseStruct->pt.x + pMouseStruct->pt.y
                    > (screen_x_real - 1) + (screen_y_real - 1) - 5)
                   && (pMouseStruct->pt.x < screen_x_real)
                   && (pMouseStruct->pt.y
                       < screen_y_real))  // Правый нижний угол (но не на втором
                                          // экране)
          {
            // Запускаем таймер, только если пришли в угол снаружи!!!
            if (1 != top_position)
            ...
            top_position = 1;
                     ...
                     && (pMouseStruct->pt.y < screen_y_real)
                     && (pMouseStruct->pt.y >= 0)
                     && (pMouseStruct->pt.x < screen_x_real)
                     && (pMouseStruct->pt.x
                         >= 0))  // Ждали окончания сигнала таймера, но уехали
                                 // из угла
          ...
          // Обычная обработка - но теперь с оговоркой: двойной щелчок правой
          // мог остановить эмуляцию!
          if (!flag_stop_emulation) {
            if (MHSettings::hh) {
              // Только в режиме 6 scroll отличается от move, остальным режимам
              // всё равно, что вызывать
              if ((right_button_down && left_button_down)
                  || flag_scroll_started) {
                MHSettings::hh->OnMouseScroll(pMouseStruct->pt.x,
                                              pMouseStruct->pt.y);
                // !!! Ошибка была !!! Кроме режима 6 никто не знает, что мышь
                // на месте стоит !!!
              nomove6:
                if (flag_stop_mouse && 6 == MHSettings::mode)
                  return 1;  // Теперь при скролле мышь не двигать, если скролл
                             // из окошка!
              }

              // Проверка возвращаемого значения была сделана из-за режима 8
              // (BFBC2) Много кода пришлось перелопатить... и не помогло...
              if (MHSettings::hh->OnMouseMove(pMouseStruct->pt.x,
                                              pMouseStruct->pt.y)) {
                // pMouseStruct->pt.x=0;
                // pMouseStruct->pt.y=0;
                return 1;
          ...
          if ((right_button_down) && (MHSettings::flag_no_move_right_mb))
            return 1;
          break;

        case WM_MBUTTONDOWN:  // У Саши Зенько роль правой кнопки выполняет
                              // колёсико!!! А настоящая правая делает только
                              // скролл
#ifdef SASHA_ZENKO
          ...
          if (MHSettings::hh) {
            ...
            // Нет, это не начало скролла, обрабатываем по обычному
              ...
              if (!flag_right_button_waits) {
                if (!MHKeypad::Press4(10, true))
                  suppress_right_mb = false;  // Если было выбрано <ничего>, то
                                              // нажимаем настоящую кнопку мыши
                // Это если мы при отпускании мыши жмём клавишу ещё раз (но
                // только если мышь реально подавилась)
                else if (MHSettings::flag_right_mb_push_twice) {
                  SetTimer(MHhwnd,
                           4,
                           MHSettings::timeout_mouse_click,
                           NULL);  // Это если мы при отпускании мыши жмём
                                   // клавишу ещё раз
                  flag_right_button_waits = true;
                }
              }
            } else
              suppress_right_mb = MHSettings::hh->OnRDown();

            // Заплаточка: двойной щелчок переводит левую кнопку мыши в другое
            // состояние
        ...
        case WM_MBUTTONUP:  // У Саши Зенько роль правой кнопки выполняет
                            // колёсико!!! А настоящая правая делает только
                            // скролл
#ifdef SASHA_ZENKO
            ...
            if (MHSettings::flag_right_mb_iskey) {
              if (MHSettings::flag_right_mb_push_twice) {
                ...
                if (!flag_right_button_waits) {
                  ...
                  if (!MHKeypad::Press4(10, true))
                    suppress_right_mb =
                        false;  // Если было выбрано <ничего>, то нажимаем
                                // настоящую кнопку мыши
                  SetTimer(MHhwnd, 4, MHSettings::timeout_mouse_click, NULL);
                  flag_right_button_waits = true;
                }
              } else if (!MHKeypad::Press4(10, false))
                suppress_right_mb = false;  // Если было выбрано <ничего>, то
                                            // нажимаем настоящую кнопку мыши
            } else
              suppress_right_mb = MHSettings::hh->OnRUp();
             ...
             // смена режима
                  {
                          ...
                          else
                          {
                                  ...
                                  break; // позволить обработать отпускание мыши
             системе, мы ещё не перехватили её нажатие, не трогаем и отпускание
                          }
                  }
          */
          /*	if(flag_scroll_started)
          ...
          if (flag_left_button_key
              && (!flag_magic_left_click))  // держали недолго
          {
            MHSettings::hh->OnLUp();
            return 1;
          }
