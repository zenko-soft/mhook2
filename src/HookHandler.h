...
class MHookHandler {
 public:
  ...
  virtual int GetPosition() {
    return MHKeypad::GetPosition();
  }  // по умолчанию возвращаем нажатую клавишу
  virtual void OnTimer(){};
  virtual void OnDraw(
      HDC hdc,
      LONG window_size){};  // Нужна в 4 режиме для рисования квадратиков
  virtual void Halt(){};
  void HaltGeneral() {
    rbutton_pressed = false;
    initialized = false;
  ...
  void Deinitialize() {
    initialized = false;
  }  // Нужно, когда мы временно не обрабатываем перемещения мыши (пауза,
     // скролл)
