...
class MHSettings {
 public:
  ...
  static void SetNumPositions(int _num_positions) {
  ...
  static void SetMouseSensitivity(int _mouse_sensitivity) {
    mouse_sensitivity = _mouse_sensitivity;
  }
  static MHookHandler *hh;
  static int GetPosition() {
    if (hh)
      return hh->GetPosition();
    else
      return -1;
  }
  ...
  static LONG minimal_mouse_speed, timeout_mouse_switch, timeout_mouse_click,
      deadx, deady;
  static bool flag_enable_speed_button;
  static bool flag_2moves, flag_2moves_mode1, flag_change_direction_ontheway,
      flag_right_mb_iskey, flag_alt2, flag_no_move_right_mb,
      flag_mode5autoclick, flag_right_mb_doubleclick, flag_left_mb_push_twice,
      flag_right_mb_push_twice, flag_downall, flag_skip_fast,
      flag_up_immediately;
...
// А это для волшебных окон, которые могут не только кнопки жать, но и ещё
// всякое
