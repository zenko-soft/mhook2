...
static int position_mem_opposite =
    0;  // Раньше путались позиции от таймера и от противоположного направления.
        // Теперь это разные переменные.

//=====================================================================================================
// Функция, которая определяет, является ли новое направление противоположным к
// старому Для 8 направлений считаем, что противоположными являются три
// направления >90 градусов от старого
//=====================================================================================================
bool IsOpposite(int old_direction, int new_direction) {
  ...
  } else  // 8 позиций
  {
    ...
    if (0 == opposite8)  // Спец место, где семёрка переходит в 0
    {
      ...
      if ((new_direction == opposite8 - 1) || (new_direction == opposite8 + 1))
        return true;
    }
  }
...
int MHookHandler1::OnMouseMove(LONG _x, LONG _y) {
  ...
  // теперь по-новому: если есть прилипание к оси, то можно только
  // противоположное или то же. Поворачивать нельзя.
  if (initialized) {
    dx = (_x - last_x);
    dy = (_y - last_y);
    ...
    if ((true == MHSettings::flag_downall)
        && (8 == MHSettings::GetNumPositions())
        && ((3 == position) || (5 == position))) {
      if (MHSettings::flag_up_immediately) goup = true;
      position = 4;
    }

    if (!rbutton_pressed)  // известно последнее положение мыши, правая кнопка
                           // не нажата
    {
      // Новая опция - из конца в конец в два движения (только при
      // 0==alt_offset)
      if ((MHSettings::flag_2moves_mode1) && (0 == alt2_offset)) {
        if (0 <= position)  // новое направление
        {
          ...
          if (IsOpposite(position_mem_opposite, position)) {
            ...
            // сбросим нажатую клавишу. Нажать противоположную сможем только
            // после таймаута
            ...
            } else  // при выставленном флаге противоположного направления
            {
              ...
              // Не дожидаясь времени, когда можно идти назад, снова пошли
              // вперёд
              ...
              if (goup)
                MHKeypad::Press(
                    position,
                    false,
                    alt2_offset);  // Флаг сказал, что нужно тут же отжать
            }
          } else  // не противоположное (position_mem содержит всё что угодно),
          {
            if (true == flag_opposite_direction) {
              // 3.
              // ждём выхода в сторону position_mem, а по дороге завернули в
              // сторону только здесь возможен поворот в сторону !!! Но только
              // если прошло время обездвиженности!!!
              if (timeGetTime() - opposite_time
                  > 100)  // была пауза, можно идти в противоположном
                          // направлении
              {
                MHKeypad::Press(position, true, alt2_offset);
                flag_opposite_direction = false;
                ...
                opposite_time = timeGetTime();  // паузы в 50 мс неподвижности
                                                // не было, перевзводим
              }
              // MHKeypad::Press(position,true);
              // flag_opposite_direction=false; // уход в сторону - сброс
              // ожиданий
          ...
          if (flag_opposite_direction) {
            if (timeGetTime() - opposite_time
                > 100)  // была пауза, можно идти в противоположном направлении
            {
              MHKeypad::Press(position_mem_opposite, true, alt2_offset);
              flag_opposite_direction = false;
              if (goup)
                MHKeypad::Press(
                    position_mem_opposite,
                    false,
                    alt2_offset);  // Флаг сказал, что нужно тут же отжать
            } else
              opposite_time = timeGetTime();  // паузы в 50 мс неподвижности не
                                              // было, перевзводим
          }
        }
        ...
        if (0 == alt2_offset) {
          ...
          if (0 <= position)  // -2=мышь ваще не двигалась, -1= направление не
                              // изменилось
          {
            MHKeypad::Press(position, true, alt2_offset);
            if (goup)
        ...
        } else  // Если это альтернативная раскладка, то взвести таймер
        {
          ...
          if (0 <= position)  // -2=мышь подвинулась на недостаточное растояние,
                              // -1= направление не изменилось
          {
            MHKeypad::Press(
                position,
                true,
                alt2_offset);  // По движению правой кнопки нажимать
                               // альтернативные клавиши из первой раскладки
          ...
          // Таймер взводим заново при любом движении мыши, если было хоть
          // что-то нажато ранее то есть -1!=position_mem
          if (-1 != position_mem) {
    ...
    else  // нажата правая кнопка. Внимание!!!! Здесь может быть 8 позиций,
          // тогда движение с правой кнопкой игнорируем !!!!
    {
      // Изменение 18.04 - позволяем альтернативным кодировкам  работать и в 8
      // позициях
      // if(4==MHSettings::GetNumPositions())
      //{
      if (!MHSettings::flag_alt2)  // Так было, пока не ввели вторую
                                   // альтернативную: движения с нажатой правой
                                   // вызывали нажатия c таймером
      {
        // обработка правой кнопки мыши
        if (0 <= position)  // -2=мышь подвинулась на недостаточное растояние,
                            // -1= направление не изменилось
        {
          // MHKeypad::Press(position,true, 6); // По движению правой кнопки
          // нажимать альтернативные клавиши из первой раскладки
          MHKeypad::Press(position, true, alt2_offset);
          position_mem = position;
        }
        // Таймер взводим заново при любом движении мыши, если было хоть что-то
        // нажато ранее то есть -1!=position_mem
        if (-1 != position_mem) {
          last_time = timeGetTime();
          SetTimer(MHhwnd, 1, MHSettings::timeout_after_move, NULL);
        }
      } else  // flag_alt2 - теперь движения с нажатой правой - это выбор
              // раскладки
      {
        // Изменение 18.04 - позволяем альтернативным кодировкам  работать и в 8
        // позициях
        ...
        else  // (8 позиций) Изменение 18.04 - позволяем альтернативным
              // кодировкам  работать и в 8 позициях
        {
...
#endif
  MHKeypad::Reset(alt2_offset);  // Отпускаем нажатые кнопки (в альтернативных
                                 // кодах клавиш, смещение 6)
  position_mem = -1;
  position_mem_opposite = -1;
  // Начинаем новый отсчет движений
  MHVector::Reset();
...
void MHookHandler1::OnDraw(HDC hdc, LONG window_size) {
  ...
  if (MHSettings::flag_alt2) {
    switch (alt2_offset) {
      case 0:
        TextOut(hdc, 90, 90, L"0", 1);
        break;

      case 11:
        TextOut(hdc, 90, 90, L"<", 1);
        break;
...
void MHookHandler1::OnTimer() {
  ...
  KillTimer(MHhwnd,
            1);  // Чтобы в отладчике не получать сообщения при обработке Press
#ifdef _DEBUG
  OutputDebugString(L"Timer keyup");
#endif
  MHKeypad::Press(
      position_mem, false, alt2_offset);  // По движению правой кнопки нажимать
                                          // альтернативные клавиши
  MHVector::Reset();  // Вот это обязательно, иначе в том же направлении мышь не
                      // нажмёт клавишу
  // Почему-то Reset не включает перерисовку
  InvalidateRect(MHhwnd, NULL, TRUE);
