...
int MHookHandler2::OnMouseMove(LONG _x, LONG _y) {
  ...
  if (initialized)  // известно последнее положение мыши, правая кнопка не
                    // нажата
  {
    dx = (_x - last_x);
    dy = (_y - last_y);
                        ...
                        // изменилось
    {
      ...
      if (rbutton_pressed)  // Если поменяли направление при нажатой правой
                            // кнопке мыши - нажать клавишу
      {
