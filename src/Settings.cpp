...
DWORD MHSettings::time_between_pushes =
    100;  // 100 миллисекунд между нажатиями на клавиши
DWORD MHSettings::timeout_after_move =
    100;  // 100 миллисекунд после последнего известия о движении мыши
LONG MHSettings::minimal_mouse_speed =
    900;  //(квадрат числа пикселов за 1/10 секунды)
LONG MHSettings::timeout_mouse_switch =
    1500;  // полторы секунды на переключение
LONG MHSettings::timeout_mouse_click =
    143;  // 1/7 секунды держим клавишу нажатой
...
bool MHSettings::flag_no_move_right_mb =
    false;  // Флаг запрещает двигать мышь, когда нажата правая кнопка
// bool MHSettings::flag_no_move_right_mb=true; // Флаг запрещает двигать мышь,
// когда нажата правая кнопка
bool MHSettings::flag_mode5autoclick = false;
bool MHSettings::flag_right_mb_doubleclick =
    false;  // Стоп эмуляции по двойному щелчку
bool MHSettings::flag_left_mb_push_twice =
    false;  // нажимать клавишу также при отпускании ЛК мыши
bool MHSettings::flag_right_mb_push_twice =
    false;  // нажимать клавишу также при отпускании ПК мыши
...
bool MHSettings::flag_up_immediately =
    false;  // Только что нажатая кнопка должна быть отжата (1 режим)
...
static MHIntChar dlg_sensitivity[MH_NUM_SENSITIVITY] = {
    {L"1", 1}, {L"5", 5}, {L"10", 10}, {L"25", 25}, {L"50", 50}, {L"100", 100}};
...
MHWORDChar dlg_scancodes[MH_NUM_SCANCODES_EXTRA] = {
    ...
    {L"ЛКМ+F12", 0xE101},
    {L"Мышь влево", 0xE102},
    {L"Мышь вправо", 0xE103},
    {L"Скролл туда", 0xE104},
    {L"Скролл сюда", 0xE105}
...
static int dlg_current_scancodes[15] = {
    27, 8, 23, 5, 67, 53, 24, 66, 12, 17, 0, 6, 9, 0, 56};

// Таймаут после движения
#define MH_NUM_TIMEOUT 9
static MHIntChar dlg_timeout[MH_NUM_TIMEOUT] = {
    {L"50 мс", 50},
    {L"75 мс", 75},
    {L"100 мс", 100},
    {L"125 мс", 125},
...
static MHIntChar dlg_speed[MH_NUM_SPEED] = {{L"100", 100},
                                            {L"200", 400},
                                            {L"300", 900},
                                            {L"400", 1600},
                                            {L"500", 2500},
...
static MHIntChar dlg_switch_timeout[MH_NUM_SWITCH_TIMEOUT] = {{L"0,1", 100},
                                                              {L"0,5", 500},
                                                              {L"1", 1000},
                                                              {L"1,5", 1000},
                                                              {L"2", 2000},
...
static MHIntChar dlg_deadzones[MH_DEAD_ZONES] = {
    {L"50", 50}, {L"100", 100}, {L"200", 200}, {L"25", 25}};
...
static MHIntChar dlg_mode3axe[MH_NUM_MODE3AXE] = {
    {L"не надо", -1}, {L"на оси вправо-влево", 0}, {L"на оси вверх-вниз", 1}};
static int dlg_current_mode3axe = 0;

// В режиме 5 выбор колесом
#define MH_NUM_CIRCLE_SCALES 3
static MHIntChar dlg_circlescales[MH_NUM_CIRCLE_SCALES] = {
    {L"не использовать", 0}, {L"50 пикселов", 50}, {L"100 пикселов", 100}};
...
static BOOL CALLBACK DlgSettingsWndProc(HWND hdwnd,
  ...
  switch (uMsg) {
    case WM_COMMAND:
      switch (LOWORD(wparam)) {
        case IDC_BUTTON_DOPLNITELNO:
          ...
          DialogBox(MHInst,
                    MAKEINTRESOURCE(IDD_DIALOG_SETTINGS2),
                    hdwnd,
                    (DLGPROC)DlgSettings2WndProc);
          MagicWindow::Hide();
          return 1;
        ...
        case IDC_BUTTON_SAVE:  // Сохраняем файл
          MHSettings::BeforeSaveOrStart(
              hdwnd);  // Текущие поля диалога копирует в переменные
    ...
    case WM_INITDIALOG:
      ...
      // Здесь не работает.
      // SetWindowLong(hdwnd,GWL_STYLE,GetWindowLong(hdwnd,GWL_STYLE) |
...
void MHSettings::FillDialogue(HWND hdwnd) {
  ...
  for (i = 0; i < MH_NUM_SENSITIVITY; i++) {
    SendDlgItemMessage(hdwnd,
                       ...
                       CB_ADDSTRING,
                       0,
                       (LPARAM)(dlg_sensitivity[i].stroka));
  }
  // SendDlgItemMessage(hdwnd,IDC_SENSITIVITY, CB_SETCURSEL,
  // dlg_current_sensitivity, 0L);

  // 2. Клавиши
  for (i = 0; i < MH_NUM_SCANCODES_EXTRA; i++) {
    SendDlgItemMessage(
        hdwnd, IDC_UP, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_RIGHT, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_DOWN, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_LEFT, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_BUTTON5, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_BUTTON6, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_UP2, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_RIGHT2, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_DOWN2, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_LEFT2, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_BUTTON7, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_UP3, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_RIGHT3, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_DOWN3, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_LEFT3, CB_ADDSTRING, 0, (LPARAM)(dlg_scancodes[i].stroka));
  }

  // 2.1. Мёртвые зоны
  for (i = 0; i < MH_DEAD_ZONES; i++) {
    SendDlgItemMessage(
        hdwnd, IDC_DEADX, CB_ADDSTRING, 0, (LPARAM)(dlg_deadzones[i].stroka));
    SendDlgItemMessage(
        hdwnd, IDC_DEADY, CB_ADDSTRING, 0, (LPARAM)(dlg_deadzones[i].stroka));
  }

  // 2.2 Режим 3 для одной из осей в режиме 4
  for (i = 0; i < MH_NUM_MODE3AXE; i++) {
    SendDlgItemMessage(
        hdwnd, IDC_MODE3AXE, CB_ADDSTRING, 0, (LPARAM)(dlg_mode3axe[i].stroka));
  }

  // 2.5. Минимальная скорость мыши
  for (i = 0; i < MH_NUM_SPEED; i++) {
    SendDlgItemMessage(
        hdwnd, IDC_SPEED, CB_ADDSTRING, 0, (LPARAM)(dlg_speed[i].stroka));
  }

  // 2.6. Число направлений
  for (i = 0; i < MH_NUM_DIRECTIONS; i++) {
    SendDlgItemMessage(
        hdwnd, IDC_DIRECTIONS, CB_ADDSTRING, 0, (LPARAM)(dlg_dirs[i].stroka));
  }
  ...
  for (i = 0; i < MH_NUM_TIMEOUT; i++) {
    SendDlgItemMessage(
        hdwnd, IDC_TIMEOUT, CB_ADDSTRING, 0, (LPARAM)(dlg_timeout[i].stroka));
  }
  ...
  for (i = 0; i < MH_NUM_SWITCH_TIMEOUT; i++) {
    SendDlgItemMessage(hdwnd,
                       ...
                       CB_ADDSTRING,
                       0,
                       (LPARAM)(dlg_switch_timeout[i].stroka));
  }
  ...
  for (i = 0; i < MH_NUM_CIRCLE_SCALES; i++) {
    SendDlgItemMessage(hdwnd,
                       ...
                       CB_ADDSTRING,
                       0,
                       (LPARAM)(dlg_circlescales[i].stroka));
  }
}
...
void MHSettings::AfterLoad(HWND hdwnd) {
  ...
  // 1. Чувствительность
  SendDlgItemMessage(
      hdwnd, IDC_SENSITIVITY, CB_SETCURSEL, dlg_current_sensitivity, 0L);

  // 2. Клавиши
  SendDlgItemMessage(hdwnd, IDC_UP, CB_SETCURSEL, dlg_current_scancodes[0], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_RIGHT, CB_SETCURSEL, dlg_current_scancodes[1], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_DOWN, CB_SETCURSEL, dlg_current_scancodes[2], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_LEFT, CB_SETCURSEL, dlg_current_scancodes[3], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_BUTTON5, CB_SETCURSEL, dlg_current_scancodes[4], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_BUTTON6, CB_SETCURSEL, dlg_current_scancodes[5], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_UP2, CB_SETCURSEL, dlg_current_scancodes[6], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_RIGHT2, CB_SETCURSEL, dlg_current_scancodes[7], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_DOWN2, CB_SETCURSEL, dlg_current_scancodes[8], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_LEFT2, CB_SETCURSEL, dlg_current_scancodes[9], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_BUTTON7, CB_SETCURSEL, dlg_current_scancodes[10], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_UP3, CB_SETCURSEL, dlg_current_scancodes[11], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_RIGHT3, CB_SETCURSEL, dlg_current_scancodes[12], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_DOWN3, CB_SETCURSEL, dlg_current_scancodes[13], 0L);
  SendDlgItemMessage(
      hdwnd, IDC_LEFT3, CB_SETCURSEL, dlg_current_scancodes[14], 0L);

  // 2.1. Мёртвые зоны
  SendDlgItemMessage(
      hdwnd, IDC_DEADX, CB_SETCURSEL, dlg_current_deadzone_x, 0L);
  SendDlgItemMessage(
      hdwnd, IDC_DEADY, CB_SETCURSEL, dlg_current_deadzone_y, 0L);

  // 2.2 Режим 3 для одной из осей в режиме 4
  SendDlgItemMessage(
      hdwnd, IDC_MODE3AXE, CB_SETCURSEL, dlg_current_mode3axe, 0L);

  // 2.5. Минимальная скорость мыши
  SendDlgItemMessage(hdwnd, IDC_SPEED, CB_SETCURSEL, dlg_current_speed, 0L);

  // 2.6. Число направлений
  // SendDlgItemMessage(hdwnd,IDC_DIRECTIONS, CB_SETCURSEL,
  // MHSettings::GetNumPositions()/5, 0L);
  SendDlgItemMessage(
      hdwnd, IDC_DIRECTIONS, CB_SETCURSEL, dlg_current_direction, 0L);
  ...
  // 4.5. Таймаут переключения левой кнопки мыши
  SendDlgItemMessage(
      hdwnd, IDC_SWITCH_TIMEOUT, CB_SETCURSEL, dlg_current_switch_timeout, 0L);

  // 4.6. Чувствительность мыши при прокрутке колеса
  SendDlgItemMessage(
      hdwnd, IDC_CIRCLE_SCALES, CB_SETCURSEL, dlg_current_circlescale, 0L);

  // 6. Разрешить ли нажатие пятой кнопки при быстром движении мышью
  if (MHSettings::flag_enable_speed_button)
    SendDlgItemMessage(hdwnd, IDC_FAST_PUSH, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(hdwnd, IDC_FAST_PUSH, BM_SETCHECK, BST_UNCHECKED, 0);

  // 7. Использовать ли в 4 режиме движение в 2 шага
  if (MHSettings::flag_2moves)
    SendDlgItemMessage(hdwnd, IDC_CHECK_2MOVES, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(hdwnd, IDC_CHECK_2MOVES, BM_SETCHECK, BST_UNCHECKED, 0);

  // 8. Использовать ли в 1 режиме движение в 2 шага
  if (MHSettings::flag_2moves_mode1)
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_2MOVES_MODE1, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_2MOVES_MODE1, BM_SETCHECK, BST_UNCHECKED, 0);

  // 9. можно ли менять направление движения на ходу
  if (MHSettings::flag_change_direction_ontheway)
    SendDlgItemMessage(hdwnd,
                       IDC_CHECK_CHANGE_DIRECTION_ONTHEWAY,
                       BM_SETCHECK,
                       BST_CHECKED,
                       0);
  else
  ...
  if (MHSettings::flag_right_mb_iskey)
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_RIGHT_MB_ISKEY, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_RIGHT_MB_ISKEY, BM_SETCHECK, BST_UNCHECKED, 0);

  // 11. две альтернативные раскладки
  if (MHSettings::flag_alt2)
    SendDlgItemMessage(hdwnd, IDC_CHECK_2ALT, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(hdwnd, IDC_CHECK_2ALT, BM_SETCHECK, BST_UNCHECKED, 0);

  // 12. автоклик в режиме 5
  if (MHSettings::flag_mode5autoclick)
    SendDlgItemMessage(hdwnd, IDC_CHECK_AUTOCLICK, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_AUTOCLICK, BM_SETCHECK, BST_UNCHECKED, 0);

  // 13. пауза по двойному щелчку
  if (MHSettings::flag_right_mb_doubleclick)
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_RIGHT_DBLCLK, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_RIGHT_DBLCLK, BM_SETCHECK, BST_UNCHECKED, 0);

  // 14. // нажимать клавишу также при отпускании ЛК мыши
  if (MHSettings::flag_left_mb_push_twice)
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_LEFT_PUSH_TWICE, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_LEFT_PUSH_TWICE, BM_SETCHECK, BST_UNCHECKED, 0);

  // 15. // нажимать клавишу также при отпускании ЛК мыши
  if (MHSettings::flag_right_mb_push_twice)
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_RIGHT_PUSH_TWICE, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_RIGHT_PUSH_TWICE, BM_SETCHECK, BST_UNCHECKED, 0);

  // 16. вниз+вбок = простол вниз (режим 1)
  if (MHSettings::flag_downall)
    SendDlgItemMessage(hdwnd, IDC_CHECK_DOWNALL, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(hdwnd, IDC_CHECK_DOWNALL, BM_SETCHECK, BST_UNCHECKED, 0);

  // 17. вниз+вбок сразу отпускать (режим 1)
  if (MHSettings::flag_up_immediately)
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_UP_IMMEDIATELY, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_UP_IMMEDIATELY, BM_SETCHECK, BST_UNCHECKED, 0);

  // 18. игнорировать быстрое движение (режим 3)
  if (MHSettings::flag_skip_fast)
    SendDlgItemMessage(hdwnd, IDC_CHECK_SKIP_FAST, BM_SETCHECK, BST_CHECKED, 0);
  else
    SendDlgItemMessage(
        hdwnd, IDC_CHECK_SKIP_FAST, BM_SETCHECK, BST_UNCHECKED, 0);
...
BOOL MHSettings::SettingsDialogue(HWND hwnd) {
  ...
  if (MHSettings::hh) {
    ...
    MHVector::Reset();
    MHKeypad::Reset();  // !!! А вдруг надо было с шифтом??? Тогда он сбросился
                        // в хук-хендлере!!!
  ...
  return_code = DialogBox(MHInst,
                          MAKEINTRESOURCE(IDD_DIALOG_SETTINGS),
                          hwnd,
                          (DLGPROC)DlgSettingsWndProc);

  if ((!return_code) && (restart_hook)) {
    // Проинициализировать хук хендлер (пока не знаю, что его нужно
    // проинициализировать) Продолжаем работать, восстанавливаем хук с
    // начальными параметрами
    flag_left_button_key = false;
    top_position = -1;
    handle =
        SetWindowsHookEx(WH_MOUSE_LL, HookProc, GetModuleHandle(NULL), NULL);
  }
...
static T_save_struct save_struct[NUM_SAVE_LINES] = {
    ...
    {"Button0",
     save_int,
     &(dlg_current_scancodes[0]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button1",
     save_int,
     &(dlg_current_scancodes[1]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button2",
     save_int,
     &(dlg_current_scancodes[2]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button3",
     save_int,
     &(dlg_current_scancodes[3]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button4",
     save_int,
     &(dlg_current_scancodes[4]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button5",
     save_int,
     &(dlg_current_scancodes[5]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    ...
    {"Button6",
     save_int,
     &(dlg_current_scancodes[6]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button7",
     save_int,
     &(dlg_current_scancodes[7]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button8",
     save_int,
     &(dlg_current_scancodes[8]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button9",
     save_int,
     &(dlg_current_scancodes[9]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button10",
     save_int,
     &(dlg_current_scancodes[10]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    ...
    {"Button11",
     save_int,
     &(dlg_current_scancodes[11]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button12",
     save_int,
     &(dlg_current_scancodes[12]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button13",
     save_int,
     &(dlg_current_scancodes[13]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},
    {"Button14",
     save_int,
     &(dlg_current_scancodes[14]),
     save_WORD,
     &dlg_scancodes,
     MH_NUM_SCANCODES},  // 16
    ...
    {"DeadzoneX",
     save_int,
     &dlg_current_deadzone_x,
     save_int,
     &dlg_deadzones,
     MH_DEAD_ZONES},
    {"DeadzoneY",
     save_int,
     &dlg_current_deadzone_y,
     save_int,
     &dlg_deadzones,
     MH_DEAD_ZONES},  // 18
    ...
    {"Mode",
     ...
     save_empty,
     NULL,
     8},  // Количество режимов (на самом деле нулевого нет, то есть 7)
    ...
    {"TimeoutSwitchLeftMB",
     save_int,
     &dlg_current_switch_timeout,
     save_int,
     dlg_switch_timeout,
     MH_NUM_SWITCH_TIMEOUT},
    // 6. Разрешить ли нажатие пятой кнопки при быстром движении мышью  и далее
    {"FastPush",
     save_bool,
     &MHSettings::flag_enable_speed_button,
     save_empty,
     0,
     0},  // 25
    ...
    {"2MovesMode1",
     save_bool,
     &MHSettings::flag_2moves_mode1,
     save_empty,
     0,
     0},
    {"ChangeDirOnTheWay",
     save_bool,
     &MHSettings::flag_change_direction_ontheway,
     save_empty,
     0,
     0},
    {"RightMBisKey",
     save_bool,
     &MHSettings::flag_right_mb_iskey,
     save_empty,
     0,
     0},
    ...
    {"Autoclick",
     save_bool,
     &MHSettings::flag_mode5autoclick,
     save_empty,
     0,
     0},
    {"CircleScale",
     save_int,
     &dlg_current_circlescale,
     save_int,
     dlg_circlescales,
     MH_NUM_CIRCLE_SCALES},
    {"RightMBDoubleClick",
     save_bool,
     &MHSettings::flag_right_mb_doubleclick,
     save_empty,
     0,
     0},
    ...
    {"LeftMBPushTwice",
     save_bool,
     &MHSettings::flag_left_mb_push_twice,
     save_empty,
     0,
     0},  // 34
    {"RightMBPushTwice",
     save_bool,
     &MHSettings::flag_right_mb_push_twice,
     save_empty,
     0,
     0},
    ...
    {"UpImmediately",
     save_bool,
     &MHSettings::flag_up_immediately,
     save_empty,
     0,
     0},
  ...
  while (1 == fscanf_s(fin, "%s", char_buf, _countof(char_buf))) {
    ...
    for (i = 0; i < NUM_SAVE_LINES; i++)  // Перебираем все возможные параметры
    {
      if (0 == strncmp(char_buf, save_struct[i].name, sizeof(char_buf) - 1)) {
        ...
        fgets(char_buf,
              sizeof(char_buf) - 1,
              fin);  // Остаток строки загоняем в буфер
        switch (ss.save_type) {
          case save_int:
            // Кроме индекса нужно считать ещё и значение одного из... N типов
            switch (ss.check_type) {
              case save_int:
                if (2 != sscanf_s(char_buf, "%d %d", &int_arg1, &int_arg2))
                  goto load_error;
                if ((int_arg1 < 0) || (int_arg1 >= ss.max_index))
                  goto load_error;
                // Проверка, что по указанному индексу лежит правлильное
                // значение
                if (((MHIntChar *)(ss.check_pointer) + int_arg1)->value
                    != int_arg2)
                  goto load_error;  // индекс не соответствует значению
                *((int *)ss.pointer) = int_arg1;  // Всё правильно, прописываем
                break;

              case save_WORD:
                if (2 != sscanf_s(char_buf, "%d %hx", &int_arg1, &WORD_arg))
                  goto load_error;
                if ((int_arg1 < 0) || (int_arg1 >= ss.max_index))
                  goto load_error;
                // Проверка, что по указанному индексу лежит правлильное
                // значение
                if (((MHWORDChar *)(ss.check_pointer) + int_arg1)->value
                    != WORD_arg)
                  goto load_error;  // индекс не соответствует значению
    ...
    if (!found) goto load_error;  // наткнулись на неизвестную строку
  }

#ifdef _DEBUG
  swprintf_s(tchar_buf,
             L"Файл конфигурации прочитан без ошибок.\r\nЧисло считанных "
             L"параметров: %d",
             num_succeeded);
...
load_error:
  swprintf_s(tchar_buf,
             L"Файл конфигурации прочитан с ошибками.\r\nВозможно, он от "
             L"другой версии программы.\r\nОднако, число успешно считанных "
             L"параметров: %d\r\n(Рекомендую сохранить конфигурацию заново)",
             num_succeeded);
  MHReportError(tchar_buf, hwnd);
  fclose(fin);
  return -1;
    ...
    switch (ss.check_type) {
      case save_int:
        fprintf(
            fout,
            "%d ",
            ((MHIntChar *)(ss.check_pointer) + *((int *)ss.pointer))->value);
        break;
      ...
      case save_WORD:
        fprintf(
            fout,
            "0x%hX ",
            ((MHWORDChar *)(ss.check_pointer) + *((int *)ss.pointer))->value);
        break;
    }
...
void MHSettings::BeforeSaveOrStart(HWND hdwnd) {
  dlg_current_sensitivity =
      SendDlgItemMessage(hdwnd, IDC_SENSITIVITY, CB_GETCURSEL, 0, 0L);
  MHSettings::SetMouseSensitivity(
      dlg_sensitivity[dlg_current_sensitivity].value);

  // 2. Кнопки
  dlg_current_scancodes[0] =
      SendDlgItemMessage(hdwnd, IDC_UP, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[1] =
      SendDlgItemMessage(hdwnd, IDC_RIGHT, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[2] =
      SendDlgItemMessage(hdwnd, IDC_DOWN, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[3] =
      SendDlgItemMessage(hdwnd, IDC_LEFT, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[4] =
      SendDlgItemMessage(hdwnd, IDC_BUTTON5, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[5] =
      SendDlgItemMessage(hdwnd, IDC_BUTTON6, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[6] =
      SendDlgItemMessage(hdwnd, IDC_UP2, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[7] =
      SendDlgItemMessage(hdwnd, IDC_RIGHT2, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[8] =
      SendDlgItemMessage(hdwnd, IDC_DOWN2, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[9] =
      SendDlgItemMessage(hdwnd, IDC_LEFT2, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[10] =
      SendDlgItemMessage(hdwnd, IDC_BUTTON7, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[11] =
      SendDlgItemMessage(hdwnd, IDC_UP3, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[12] =
      SendDlgItemMessage(hdwnd, IDC_RIGHT3, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[13] =
      SendDlgItemMessage(hdwnd, IDC_DOWN3, CB_GETCURSEL, 0, 0L);
  dlg_current_scancodes[14] =
      SendDlgItemMessage(hdwnd, IDC_LEFT3, CB_GETCURSEL, 0, 0L);
  ...
  // 2.1. Мёртвые зоны
  dlg_current_deadzone_x =
      SendDlgItemMessage(hdwnd, IDC_DEADX, CB_GETCURSEL, 0, 0L);
  MHSettings::deadx = dlg_deadzones[dlg_current_deadzone_x].value;
  dlg_current_deadzone_y =
      SendDlgItemMessage(hdwnd, IDC_DEADY, CB_GETCURSEL, 0, 0L);
  MHSettings::deady = dlg_deadzones[dlg_current_deadzone_y].value;

  // 2.2 Режим 3 для одной из осей в режиме 4
  dlg_current_mode3axe =
      SendDlgItemMessage(hdwnd, IDC_MODE3AXE, CB_GETCURSEL, 0, 0L);
  ...
  // 2.6. Число направлений
  dlg_current_direction =
      SendDlgItemMessage(hdwnd, IDC_DIRECTIONS, CB_GETCURSEL, 0, 0L);
  ...
  if (BST_CHECKED == SendDlgItemMessage(hdwnd, IDC_RADIO1, BM_GETCHECK, 0, 0)) {
    ...
    MHSettings::hh = &hh1;
             ...
             == SendDlgItemMessage(hdwnd, IDC_RADIO2, BM_GETCHECK, 0, 0)) {
    MHSettings::mode = 2;
    MHSettings::hh = &hh2;
  } else if (BST_CHECKED
             == SendDlgItemMessage(hdwnd, IDC_RADIO3, BM_GETCHECK, 0, 0)) {
    MHSettings::mode = 3;
    MHSettings::hh = &hh3;
  } else if (BST_CHECKED
             == SendDlgItemMessage(hdwnd, IDC_RADIO4, BM_GETCHECK, 0, 0)) {
    MHSettings::mode = 4;
    MHSettings::hh = &hh4;
    // Важно!!! В 4 режиме принудительно выставить 8 позиций!!!
    MHSettings::SetNumPositions(8);
    dlg_current_direction = 1;
  } else if (BST_CHECKED
             == SendDlgItemMessage(hdwnd, IDC_RADIO5, BM_GETCHECK, 0, 0)) {
    MHSettings::mode = 5;
    MHSettings::hh = &hh5;
    // Важно!!! В 5 режиме принудительно запрещается движение курсора при правом
    // нажатии!!
    MHSettings::flag_no_move_right_mb = true;
    MHSettings::SetNumPositions(8);
    dlg_current_direction = 1;  // и принудительно выставить 8 позиций!!!
  } else if (BST_CHECKED
             == SendDlgItemMessage(hdwnd, IDC_RADIO6, BM_GETCHECK, 0, 0)) {
    MHSettings::mode = 6;
    MHSettings::hh = &hh6;
    // Важно!!! В 6 режиме принудительно выставить 4 позиций!!!
    MHSettings::SetNumPositions(4);
    dlg_current_direction = 0;
  } else if (BST_CHECKED
             == SendDlgItemMessage(hdwnd, IDC_RADIO7, BM_GETCHECK, 0, 0)) {
    MHSettings::mode = 7;
    MHSettings::hh = &hh7;
  }
  ...
  // 4. Таймаут
  dlg_current_timeout =
      SendDlgItemMessage(hdwnd, IDC_TIMEOUT, CB_GETCURSEL, 0, 0L);
  MHSettings::timeout_after_move = dlg_timeout[dlg_current_timeout].value;

  // 4.5. Таймаут переключения режима левой кнопки мыши
  dlg_current_switch_timeout =
      SendDlgItemMessage(hdwnd, IDC_SWITCH_TIMEOUT, CB_GETCURSEL, 0, 0L);
  MHSettings::timeout_mouse_switch =
      dlg_switch_timeout[dlg_current_switch_timeout].value;

  // 4.6.
  dlg_current_circlescale =
      SendDlgItemMessage(hdwnd, IDC_CIRCLE_SCALES, CB_GETCURSEL, 0, 0L);
  MHSettings::circle_scale_factor =
      dlg_circlescales[dlg_current_circlescale].value;

  // 6. Разрешить ли нажатие пятой кнопки при быстром движении мышью
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_FAST_PUSH, BM_GETCHECK, 0, 0))
  ...
  // 7. Использовать ли в 4 режиме движение в 2 шага
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_CHECK_2MOVES, BM_GETCHECK, 0, 0))
  ...
  // 8. Использовать ли в 1 режиме движение в 2 шага
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_CHECK_2MOVES_MODE1, BM_GETCHECK, 0, 0))
    MHSettings::flag_2moves_mode1 = true;
  else
    MHSettings::flag_2moves_mode1 = false;

  // 9. можно ли менять направление движения на ходу
  if (BST_CHECKED
      == SendDlgItemMessage(
          hdwnd, IDC_CHECK_CHANGE_DIRECTION_ONTHEWAY, BM_GETCHECK, 0, 0))
  ...
  // 10. правая кнопка мыши вместо обычного поведения ведёт себя, как клавиша
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_CHECK_RIGHT_MB_ISKEY, BM_GETCHECK, 0, 0))
  ...
  // 11. две альтернативные раскладки
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_CHECK_2ALT, BM_GETCHECK, 0, 0))
    MHSettings::flag_alt2 = true;
  else
    MHSettings::flag_alt2 = false;

  // 12. автоклик в режиме 5
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_CHECK_AUTOCLICK, BM_GETCHECK, 0, 0))
  ...
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_CHECK_RIGHT_DBLCLK, BM_GETCHECK, 0, 0))
  ...
  if (BST_CHECKED
      == SendDlgItemMessage(
          hdwnd, IDC_CHECK_LEFT_PUSH_TWICE, BM_GETCHECK, 0, 0))
  ...
  if (BST_CHECKED
      == SendDlgItemMessage(
          hdwnd, IDC_CHECK_RIGHT_PUSH_TWICE, BM_GETCHECK, 0, 0))
  ...
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_CHECK_DOWNALL, BM_GETCHECK, 0, 0))
  ...
  // 17. вниз+вбок сразу отпускать (режим 1)
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_CHECK_UP_IMMEDIATELY, BM_GETCHECK, 0, 0))
  ...
  // 18. игнорировать быстрое движение (режим 3)
  if (BST_CHECKED
      == SendDlgItemMessage(hdwnd, IDC_CHECK_SKIP_FAST, BM_GETCHECK, 0, 0))
