...
extern HWND MHhwnd;
extern bool
    flag_stop_mouse;  // Останавливаем мышь, только начав дрег внутри окна !!!
...
void MHookHandler6::OnMouseScroll(LONG _x, LONG _y) {
  ...
  if (initialized)  // известно последнее положение мыши
  {
    ...
    if ((1 == position)
        || (3 == position))  // -2=мышь подвинулась на недостаточное растояние,
    ...
    // Таймер взводим заново при любом движении мыши (кроме скролла), если было
    // хоть что-то нажато ранее то есть -1!=position_mem
    if ((-1 != position_mem)
        && ((1 == position) || (3 == position) || (-1 == position)
            || (-2 == position))) {
...
//======================================================================================================
// Здесь используется только для того, чтобы сохранялся режим initialized (не
// терять dx, dy)
//======================================================================================================
