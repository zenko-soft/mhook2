...
class MagicWindow {
 public:
  int myindex;  // Нужно, когда общается с диалогом установок (похоже, больше не
                // используется)
  ...
  int mw_group;  // группа взаимоисключающих клавиш, от 1 до 4 (или 0=нет
                 // группы)
  bool pressed;
  bool f_inside_window;
  int flag_ignore_mouse_move;  // При спрятывании и появлении окна генерируется
                               // ложное mouse_move
  TRACKMOUSEEVENT tme;
  ...
  void PressSpecial(BYTE operation);  // вместо нажатия на клавишу выполняет
                                      // какую-нибудь хитрую операцию

  static bool editmode;
  static RECT
      adjust_rect;  // корректировка размеров окна с учетом заголовка и рамки
 protected:
