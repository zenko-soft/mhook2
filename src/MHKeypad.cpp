...
void MHKeypad::Init(WORD _scancode0,
                    WORD _scancode1,
                    WORD _scancode2,
                    WORD _scancode3,
                    WORD _scancode4,
  ...
  if (-1 != keypad_position)
    Reset();  // Перед переинициализацией отпустить кнопки
...
bool MHKeypad::Press4(int position, bool down, int shift) {
  ...
  if ((position < 0)
      || (position > 10))  // Теперь бывает и 5, и 10 (на правой кнопке мыши)
  ...
  if (down)
    button_pressed[position + shift] = 1;
  else
    button_pressed[position + shift] = 0;
#endif
...
void MHKeypad::Press(int position, bool down, int shift) {
  ...
  if (4 == MHSettings::GetNumPositions()) {
    if (true == down) {
      Reset(shift);  // при нажатии отпустить все отсальные. А при отпускании не
                     // делать этого.
  ...
  } else  // 8 позиций, отпускаем до двух клавиш, нажимаем до двух клавиш
  {
    if (true == down) {
      for (int i = 0; i < 4; i++) {
        if (-1 != keypad_position)  // сравниваем нажатое и новое направления по
                                    // 4 клавишам
        {
...
void MHKeypad::Press8(int position, bool down) {
  if (true == down) {
    ...
    if (keypad_position != -1) {
      if (keypad_position > 3)
        Press4(keypad_position - 4, false, 6);
      else
        Press4(keypad_position, false, 0);
    }
    keypad_position = position;
    // OutputDebugString("Нажали\n");
  } else {
  ...
  if (position > 3)
    Press4(position - 4, down, 6);
  else
    Press4(position, down, 0);
