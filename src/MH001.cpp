...
// Командную строку не обрабатываем
{
  ...
  hfont = CreateFont(
      -32, 0, 0, 0, FW_BOLD, 0, 0, 0, RUSSIAN_CHARSET, 0, 0, 0, 0, L"Arial");
  // Кисти
  ...
  handle = SetWindowsHookEx(WH_MOUSE_LL, HookProc, GetModuleHandle(NULL), NULL);
  ...
  if (G_eytracker_is_working)  // выключаем
  {
    ...
    if (0 == G_eytracker_num)
      BKBTobiiREX::Halt(NULL);
    else
      BKBTET::Halt(NULL);
  }
