...
volatile bool flag_ShutDownThreads =
    false;  // flag to inform the threads to terminate
...
unsigned __stdcall HeartBeatThread(void *p) {
  while (!flag_ShutDownThreads) {
    if (SOCKET_ERROR
        == send(TETSocket, JSON_heart_beat, strlen(JSON_heart_beat), 0))
...
unsigned __stdcall ReaderThread(void *p) {
  ...
  float x_avg, y_avg, left_x_avg, left_y_avg, left_pcenter_x, left_pcenter_y,
      left_psize, left_x_raw, left_y_raw, x_raw, y_raw, right_x_avg,
      right_y_avg, right_pcenter_x, right_pcenter_y, right_psize, right_x_raw,
      right_y_raw;
  ...
  while (!flag_ShutDownThreads) {
    ...
    if (20 == num_scanned) {
      ...
      if (7 == state) {
        ...
        // if(7==state) gd.tracking_status =
        // TOBIIGAZE_TRACKING_STATUS_BOTH_EYES_TRACKED; else gd.tracking_status
