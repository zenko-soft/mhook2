...
void on_gaze_data(toit_gaze_data *gazedata,
                  void *user_data);  // Определён в OnGazeData.cpp
...
typedef void (*type_tobii_device_url_receiver)(char const *url,
                                               void *user_data);
typedef void (*type_tobii_gaze_point_callback)(
    type_tobii_gaze_point const *gaze_point, void *user_data);

typedef __declspec(dllimport) int(__cdecl *type_tobii_api_create)(to_api **api,
                                                                  void *,
                                                                  void *);
typedef __declspec(dllimport) int(
    __cdecl *type_tobii_enumerate_local_device_urls)(
    to_api *api, type_tobii_device_url_receiver receiver, void *user_data);
typedef __declspec(dllimport) int(__cdecl *type_tobii_device_create)(
    to_api *api, char const *url, to_device **device);
typedef __declspec(dllimport) int(__cdecl *type_tobii_gaze_point_subscribe)(
    to_device *device,
    type_tobii_gaze_point_callback callback,
    void *user_data);
typedef __declspec(dllimport) int(__cdecl *type_tobii_gaze_point_unsubscribe)(
    to_device *device);
typedef __declspec(dllimport) int(__cdecl *type_tobii_device_destroy)(
    to_device *device);
typedef __declspec(dllimport) int(__cdecl *type_tobii_api_destroy)(to_api *api);
typedef __declspec(dllimport) int(__cdecl *type_tobii_wait_for_callbacks)(
    to_engine *engine, int device_count, to_device *const *devices);
typedef __declspec(dllimport) int(__cdecl *type_tobii_device_process_callbacks)(
    to_device *device);
...
static void url_receiver(char const *url, void *user_data) {
  ...
  if (strlen(url) < 256) strcpy_s(buffer, 255, url);
}

//=========================================================
// Callback function
//=========================================================
static void gaze_point_callback(type_tobii_gaze_point const *gaze_point,
                                void *user_data) {
...
unsigned __stdcall TobiiStreamThread(void *p) {
  ...
  while (!flag_stop_thread) {
    ...
    if (err) {
      MHReportError(
          err, __WIDEFILE__, L"tobii_device_process_callbacks", __LINE__);
      return 0;
    }
  }
  return 0;
...
int BKBTobiiREX::Init(HWND hwnd) {
  ...
  if (0 == TobiiStreamDLL) {
    MHReportError(
        L"Не удалось загрузить библиотеку "
        L"tobii_stream_engine.dll\r\nСкопируйте её в рабочий каталог программы",
        hwnd);
    return 1;
  }
  ...
  fp_tobii_api_create =
      (type_tobii_api_create)GetProcAddress(TobiiStreamDLL, "tobii_api_create");
  fp_tobii_enumerate_local_device_urls =
      (type_tobii_enumerate_local_device_urls)GetProcAddress(
          TobiiStreamDLL, "tobii_enumerate_local_device_urls");
  fp_tobii_device_create = (type_tobii_device_create)GetProcAddress(
      TobiiStreamDLL, "tobii_device_create");
  fp_tobii_gaze_point_subscribe =
      (type_tobii_gaze_point_subscribe)GetProcAddress(
          TobiiStreamDLL, "tobii_gaze_point_subscribe");
  fp_tobii_gaze_point_unsubscribe =
      (type_tobii_gaze_point_unsubscribe)GetProcAddress(
          TobiiStreamDLL, "tobii_gaze_point_unsubscribe");
  fp_tobii_device_destroy = (type_tobii_device_destroy)GetProcAddress(
      TobiiStreamDLL, "tobii_device_destroy");
  fp_tobii_api_destroy = (type_tobii_api_destroy)GetProcAddress(
      TobiiStreamDLL, "tobii_api_destroy");
  fp_tobii_wait_for_callbacks = (type_tobii_wait_for_callbacks)GetProcAddress(
      TobiiStreamDLL, "tobii_wait_for_callbacks");
  fp_tobii_device_process_callbacks =
      (type_tobii_device_process_callbacks)GetProcAddress(
          TobiiStreamDLL, "tobii_device_process_callbacks");

  // Трюк - старый указатель на новую функцию (чтобы не переделывать BKBRepError
  // )
  fp_tobiigaze_get_error_message =
      (type_tobiigaze_get_error_message)GetProcAddress(TobiiStreamDLL,
                                                       "tobii_error_message");

  if (!fp_tobii_api_create || !fp_tobii_enumerate_local_device_urls
      ...
      || !fp_tobiigaze_get_error_message) {
    MHReportError(
        L"Не удалось получить необходимые функции из tobii_stream_engine.dll",
        hwnd);
    return 1;
  }
  ...
  if (err) {
    MHReportError(
        err, __WIDEFILE__, L"tobii_gaze_point_subscribe", __LINE__, hwnd);
    return 1;
  }

  // Здесь создаём поток для опроса айтрекера
  tobii_thread_handler =
      _beginthreadex(NULL, 0, TobiiStreamThread, NULL, 0, NULL);
...
int BKBTobiiREX::Halt(HWND hwnd) {
  ...
  if (err) {
    MHReportError(
        err, __WIDEFILE__, L"tobii_gaze_point_unsubscribe", __LINE__, hwnd);
