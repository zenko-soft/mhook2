...
void MHookHandler::OnFastMove(LONG _dx, LONG _dy) {
  ...
  // Наконец, попали в нужный интервал времени. Если скорость мыши нас устроит,
  // нажимаем клавишу.
  mouse_path_squared += dx * dx + dy * dy;
  if (mouse_path_squared
      > MHSettings::minimal_mouse_speed)  // Потом подберём значения
  {
...
void MHookHandler::OnMouseScroll(LONG _x, LONG _y) {
  OnMouseMove(_x, _y);  // По умолчанию все хендлеры не обращают внимание на то,
                        // что нажата не только правая, но и левая
}
...
void MHookHandler::OnLDown() {
  ...
  if (!flag_left_button_waits) {
    ...
    // if(MHSettings::flag_left_mb_push_twice) MHKeypad::Press4(5,false); // Это
    // если мы при отпускании мыши жмём клавишу ещё раз
    if (MHSettings::flag_left_mb_push_twice) {
      SetTimer(MHhwnd,
               3,
               MHSettings::timeout_mouse_click,
               NULL);  // Это если мы при отпускании мыши жмём клавишу ещё раз
...
void MHookHandler::OnLUp() {
  if (MHSettings::flag_left_mb_push_twice) {
    if (!flag_left_button_waits) {
      MHKeypad::Press4(
          5, true);  // Это если мы при отпускании мыши жмём клавишу ещё раз
